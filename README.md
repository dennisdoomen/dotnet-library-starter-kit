...
GitVersion requires access to the full git history to calculate the correct semantic version. If you are using Azure Pipelines, make sure to **disable the Shallow Fetch option** in your pipeline settings. You can find this under the pipeline's settings in the Azure DevOps UI under *General* → uncheck *Shallow fetch*. Without this, GitVersion will fail because it cannot traverse the full commit history.
